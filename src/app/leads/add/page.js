...
function LeadFormContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = isEditing ? '/api/leads' : '/api/leads';
      const method = isEditing ? 'PUT' : 'POST';
      
      const submitData = { ...formData };
      if (isEditing) {
        submitData.id = leadId;
      }
          ...
          <div className="header-content">
            <div className="header-text">
              <h1 className="page-title">
                {isEditing ? "✏️ Edit Lead" : "➕ Create New Lead"}
              </h1>
              <p className="page-description">
                {isEditing ? "Update lead information and track progress" : "Add a new lead to your pipeline"}
              </p>
            </div>
            <div className="header-actions">
              ...
              >
                {saving ? "Saving..." : (isEditing ? "💾 Update Lead" : "💾 Save Lead")}
              </button>
            </div>
          </div>
        </div>
